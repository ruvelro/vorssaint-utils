...
private struct DockPreviewPanelContent: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !windows.isEmpty, let mediaPlayer {
                ...
                .frame(height: DockPreviewSupport.mediaPanelHeight)
                    ...
                    ScrollView(stacksVertically ? .vertical : .horizontal, showsIndicators: false) {
                        cardRun {
                            ForEach(windows) { window in
                                ...
                                .onHover { hovering in
                                    if hovering { onPreview(window) } else { onEndPreview(window) }
                                }
                            }
                        }
                        .padding(.horizontal, DockPreviewSupport.panelPadding)
                        .padding(.bottom, DockPreviewSupport.panelPadding)
                        .padding(.top, showsHeader || mediaPlayer != nil
                                 ? 0 : DockPreviewSupport.panelPadding)
                    }
        ...
        .frame(width: stacksVertically ? panelContentWidth : nil,
    ...
    private var panelContentWidth: CGFloat {
        max(DockPreviewSupport.cardWidth + DockPreviewSupport.panelPadding * 2,
            mediaPlayer == nil ? 0 : DockPreviewSupport.mediaPanelWidth)
    }
    ...
    private var cardRunHeight: CGFloat {
        DockPreviewSupport.cardHeight + DockPreviewSupport.panelPadding * 2
            ...
            + (mediaPlayer == nil ? 0 : DockPreviewSupport.mediaPanelHeight)
    }
