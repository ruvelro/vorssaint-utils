...
final class BrightnessService: ObservableObject {
    ...
    func refresh(force: Bool = false) {
        ...
        workQueue.async { [weak self] in
            self?.rebuild(generation: generation, previousDisplays: previousDisplays,
                          hdrStates: hdrStates, screenNames: screenNames)
        }
    }
    ...
    /// True when HDR has moved this display's brightness route to gamma
    /// dimming. Extra Brightness uses this to avoid composing a boost overlay
    /// with a gamma curve that is simultaneously reducing the same display.
    func hdrOwnsLuminance(for id: CGDirectDisplayID) -> Bool {
        displays.first { $0.id == id }?.hdrEnabled == true
    }
    ...
    private func rebuild(generation: Int, previousDisplays: [BrightnessDisplay],
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if UserDefaults.standard.bool(forKey: DefaultsKey.extraBrightnessEnabled) {
                ExtraBrightnessService.shared.displayLuminanceOwnershipDidChange()
            }
        }
    }
...
private final class HDRBridge {
    ...
    func setHDREnabled(_ enabled: Bool, for id: CGDirectDisplayID) -> Bool {
        ...
        return isHDREnabled(id) == enabled
    }
}
