...
enum DockPreviewSupport {
    ...
    /// them, so apps such as Music or a browser never lose their normal Dock
    /// preview while media is active.
    static func combinedMediaPanelSize(windowPanelSize: CGSize,
                                       screenVisibleFrame: CGRect) -> CGSize {
        let availableWidth = max(1, screenVisibleFrame.width - edgePadding * 2)
        let availableHeight = max(1, screenVisibleFrame.height - edgePadding * 2)
        return CGSize(width: min(max(windowPanelSize.width, mediaPanelWidth), availableWidth),
                      height: min(windowPanelSize.height + mediaPanelHeight, availableHeight))
    }
